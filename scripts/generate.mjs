/**
 * generate.mjs（免費版，不需要任何金鑰）
 * 每天執行一次：抓取免費的 Google 新聞 RSS，依類別 × 語言整理成 news.json。
 * 前端網頁只讀這個檔案。整條流程沒有 API 金鑰、沒有費用。
 *
 * 本機測試： node scripts/generate.mjs
 */
...
// 想增減類別、改顏色，改這裡就好（前端會自動依此產生分類）
// labels 同時是各語言的「顯示名稱」與「搜尋關鍵字」
const CATEGORIES = [
